...
"""
No pretraining: Test accuracy=0.8272, Task-only accuracy (baseline chunks excluded): 0.6552
20 epoch pretraining: Test accuracy=0.8088, Task-only accuracy (baseline chunks excluded): 0.6318
50 epoch pretraining: Test accuracy=0.8347, Task-only accuracy (baseline chunks excluded): 0.6708
100 epoch pretraining: Test accuracy=0.8301, Task-only accuracy (baseline chunks excluded): 0.6604
50 epoch pretraining + better data augmentation + dropout in classfier: Test accuracy=0.8541, Task-only accuracy (baseline chunks excluded): 0.7078
...
class ViewEncoder(nn.Module):
    ...
    def forward(self, x):            # x: (N, C_view, 640)
        h = self.net(x.unsqueeze(1))  # (N, f2, 1, 20)
