...
class EEGNet(nn.Module):
    ...
    def __init__(self, n_classes, chans, samples,
        ...
        # train_multiview_transformer.py) -- that Linear(flat -> d_model)
        # is what dominates ViewEncoder's parameter count, so exposing the
        # same layer here is what lets EEGNet's capacity be dialed to match
        # it (see --d-model). None keeps the original paper's design:
        # classify straight off the flattened conv features.
...
def main():
    ...
    ap.add_argument("--d-model", type=int, default=128,
                     help="Bottleneck Linear(flat_features -> d_model) inserted before the "
                          ...
                          "against theirs (default 128 lands at ~43.2k params, the same ballpark "
                          "as a single-view ViewEncoder). 0 disables it: classify directly off the "
                          "flattened conv features, the original paper's design.")
