...
"""EEGNet, WITHOUT the cleaning pipeline, restricted to left_fist vs
right_fist only (binary classification).

Combines train_eegnet-raw.py's ablation (no montage/notch/ICA/bandpass)
with the left_fist-vs-right_fist class restriction: same model, same
RandAugment/SMOTE, raw uncleaned signal, just filtered down to two
classes (remapped to labels 0/1) right after loading, via the shared
