...
"""
MLP restricted to left_fist vs right_fist only (binary classification).

Same model, same training loop, same data pipeline as train_mlp.py --
the only change is filtering the built 5-class dataset down to just
these two classes (remapped to labels 0/1) right after loading, via
the shared filter_to_classes() utility.
"""
...
def main():
    ...
    model = MLP(input_dim, args.hidden_dim, len(LR_CLASSES), args.dropout).to(device)
